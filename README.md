![EVAC-OPS logo](imgs/evac-logo.png)

# Evacuation Management, Tactical Routing & Space Data Simulation Application
...
- **4-Panel Tactical Cockpit Layout (with Independently Collapsible Panels)**:
  - **Left Control Panel (`25% Width × 100% Height`, Collapsible via `#btn-toggle-left-panel`)**: Branded at the top with [`imgs/evac-logo.png`](./imgs/evac-logo.png) and collapses independently into a `38px` vertical rail to maximize horizontal map space. Contains the Scenario selector (`Brussels`, `Paris`, `Custom`) followed by **three top-justified collapsible sections (all starting collapsed by default on application startup)**:
    1. **Parameters (`#toggle-parameters-section`)**: Entity tabs and CRUD management for `Sources`, `Targets`, `No-Go`, and `Vehicles`.
    2. **Execution & Simulation (`#toggle-execution-section`)**: `Compute evacuation routes`, `Run simulation`, `Pause simulation`, `Reset simulation`, **`Simulation report`** (`#btn-simulation-report`, enabled only when the simulation is paused — opens a centered browser-window popup with a full breakdown of people evacuated to shelter, people still not evacuated per population behaviour `obedient`/`autonomous`/`random`, mean evacuation time per person overall and per behaviour, people evacuated at each pickup location, mean vehicle wait time at each pickup location, shelter capacity headroom, throughput velocity, and JSON export), and playback speed multiplier.
    3. **Space Data (`#toggle-space-data-section`)**: Sentinel-2 Optical RGB, Sentinel-1 SAR (`VV`, `VH`, `VV/VH`), `Planet Scope` (with `"Not yet available"` popup dialog), and CEMS GloFAS River Discharge forecast controls.
  - **Center Map Viewport (`Dynamic Flex Width & Height`, Default `50% Width × 75% Height`, Expands up to Full Viewport)**: Automatically expands horizontally and vertically (`ResizeObserver` + `map.invalidateSize()`) whenever the Left, Bottom, or Right panels are collapsed. Features zero-API-key OpenStreetMap base layers (`Standard OSM`, `Humanitarian OSM`, `CyclOSM`), interactive polygon drawing & vehicle staging depot placement, **Blue Square Pickup Locations**, strict No-Go-avoiding directional evacuation corridors, 60 FPS HTML5 Canvas thermal density heatmap overlay, **Sentinel-2 Optical RGB** & **Sentinel-1 SAR (`VV`, `VH`, `VV/VH`)** Earth Engine layers, and **CEMS GloFAS 24h / 48h / 72h River Discharge** overlays where pixel values below `10 m³/s` appear as **fully transparent** and only pixel values above `10 m³/s` (clipped at `80`, White &rarr; Red color map) are rendered and controlled by their corresponding transparency sliders.
  - **Bottom Console Panel (`25% Height`, Collapsible via `#btn-toggle-bottom-panel`)**: Collapses independently into a `36px` status bar (retaining the event counter badge and a live preview of the latest log message) to give full vertical height to the Center Map. Displays the real-time system, routing, simulation, Earth Engine, and CEMS GloFAS log stream with level filtering.
  - **Right Situational Telemetry Panel (`25% Width × 100% Height`, Collapsible via `#btn-toggle-right-panel`)**: Collapses independently into a `38px` vertical rail (displaying a live evacuation progress percentage badge) to expand the Center Map horizontally. Shows exact population conservation KPIs (`Safe at Shelter`, `On Vehicles`, `In Source Area`), elapsed simulation clock (`MM:SS`), Target Shelter occupancy bars, and behavioral breakdowns (`Obedient`, `Autonomous`, `Random`).
